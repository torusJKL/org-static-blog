...
(defcustom org-static-blog-post-filename-function
  #'org-static-blog-generate-post-filename
  "Function to generate the HTML filename for a post.
The function receives two arguments: the post-filename and a metadata alist.
The metadata alist contains: filename, title, date, tags, description.
Default: preserves original .org -> .html behavior."
  :type 'function)
...
(defun org-static-blog-get-post-metadata (post-filename)
  "Collect post metadata into an alist for filename generation.
Returns: ((filename . \"post.org\") (title . \"My Post\") (date . <time>)
          (tags . (\"emacs\" \"blog\")) (description . \"...\"))"
  `((filename . ,(file-name-nondirectory post-filename))
    (title . ,(org-static-blog-get-title post-filename))
    (date . ,(org-static-blog-get-date post-filename))
    (tags . ,(org-static-blog-get-tags post-filename))
    (description . ,(org-static-blog-get-description post-filename))))

(defun org-static-blog-generate-post-filename (post-filename metadata)
  "Default filename generator. Changes .org extension to .html.
POST-FILENAME is the full path to the org file being published.
METADATA is an alist with post information (filename, title, date, tags, description)."
  (concat (file-name-sans-extension (alist-get 'filename metadata))
          ".html"))
...
(defun org-static-blog--generate-filename-with-metadata (relative-path post-filename)
  "Generate filename using RELATIVE-PATH and POST-FILENAME.
Helper function that handles the common logic of extracting directory,
base filename, and calling the customizable filename function."
  (let* ((relative-dir (file-name-directory relative-path))
         (base-filename (file-name-nondirectory relative-path))
         (metadata (org-static-blog-get-post-metadata post-filename)))
    ;; Update metadata with the base filename for reference
    (setf (alist-get 'filename metadata) base-filename)
    ;; Combine directory structure with user-generated filename
    (concat relative-dir
            (funcall org-static-blog-post-filename-function post-filename metadata))))
...
The filename is generated by `org-static-blog-post-filename-function`."
  (let* ((true-post-filename (file-truename post-filename))
         ...
         (relative-path (file-relative-name true-post-filename root-dir)))
    (org-static-blog--generate-filename-with-metadata relative-path post-filename)))
...
will return 'my-life-update.html'."
  (let ((relative-path (file-relative-name post-filename org-static-blog-posts-directory)))
    (org-static-blog--generate-filename-with-metadata relative-path post-filename)))
